        ...
        <div class="relative w-full max-w-xl mr-6 focus-within:text-purple-500">
            ...
            <form action="{{ route('payment_methods.index') }}" method="get">
                ...
                <input
                    ...
                    placeholder="Search for Payment Methods" autofocus aria-label="Search" />
            </form>
        </div>
                ...
                <tbody class="bg-white divide-y dark:divide-gray-700 dark:bg-gray-800">
                    @forelse ($payment_methods as $payment_method)
                        <tr class="text-gray-700 dark:text-gray-400">
                            ...
                            <td class="px-4 py-3">
                                <p class="font-semibold text-sm">{{ $payment_method->name }}</p>
                            </td>
                            <td class="px-4 py-3 text-sm">
                                {{ Str::limit($payment_method->description, 50) }}
                            </td>
                            <td class="px-4 py-3 text-sm">
                                <span
                                    class="px-2 py-1 font-semibold leading-tight {{ $payment_method->status == 'active' ? 'text-green-700 bg-green-100 rounded-full dark:bg-green-700 dark:text-green-100' : 'text-red-700 bg-red-100 rounded-full dark:text-red-100 dark:bg-red-700' }}">
                            ...
                            <td class="px-4 py-3">
                                <div class="flex items-center space-x-4 text-sm">
                                    ...
                                    <button onclick="confirmDelete('{{ $payment_method->id }}')"
                                        class="flex items-center justify-between px-2 py-2 text-sm font-medium leading-5 text-purple-600 rounded-lg dark:text-gray-400 focus:outline-none focus:shadow-outline-gray"
                                        aria-label="Delete">
                                        <svg class="w-5 h-5" aria-hidden="true" fill="currentColor" viewBox="0 0 20 20">
                                            <path fill-rule="evenodd"
                                    ...
                                    <form id="delete-form-{{ $payment_method->id }}"
                                        action="{{ route('payment_methods.destroy', $payment_method->id) }}" method="post"
                                        style="display: none;">
                                        @csrf
                                        @method('DELETE')
                                    </form>
