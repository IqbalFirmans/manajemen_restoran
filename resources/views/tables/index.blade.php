    ...
    <div class="relative w-full max-w-xl mr-6 focus-within:text-purple-500">
        ...
        <form action="{{ route('tables.index') }}" method="get">
            <input
                ...
                placeholder="Search for Tables" autofocus aria-label="Search" />
        </form>
    </div>
    ...
    <div class="w-full overflow-hidden rounded-lg shadow-xs mb-5">
        <div class="p-2">
            <p class="text-sm text-gray-600">
                Page {{ $tables->currentPage() }} of {{ $tables->lastPage() }}
            </p>
        </div>
                    ...
                    <tr class="text-gray-700">
                            ...
                            <p class="font-semibold text-sm ">
                                <span
                                    class="px-2 py-1 font-semibold leading-tight
                                        @if ($table->status == 'available') text-green-700 bg-green-100 rounded-full
                                        @elseif($table->status == 'occupied') text-red-700 bg-red-100 rounded-full
                                        @elseif($table->status == 'reserved') text-orange-700 bg-orange-100 rounded-full @endif">
                                    {{ $table->status }}
                                </span>
                            </p>
                        </td>
                        ...
                        <td class="px-4 py-3">
                            <div class="flex items-center space-x-4 text-sm">
                                ...
                                <button onclick="confirmDelete('{{ $table->id }}')"
                                    class="flex items-center justify-between px-2 py-2 text-sm font-medium leading-5 text-purple-600 rounded-lg focus:outline-none focus:shadow-outline-gray"
                                    aria-label="Delete">
                                    <svg class="w-5 h-5" aria-hidden="true" fill="currentColor" viewBox="0 0 20 20">
                                        <path fill-rule="evenodd"
